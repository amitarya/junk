...
void
LOCALNS::Tar::_size(void*
		header,unsigned
		long
		fileSize)
{
	std::sprintf(TARHEADER->size,"%011llo",(long
				long
				unsigned
				int)fileSize);
}
...
void
LOCALNS::Tar::_endRecord(std::size_t
		len)
{
	char
		c='\0';
	...
	LOCALNS::Tar::Tar(std::ostream&
			out):_finished(false),out(out)
{
...
LOCALNS::Tar::finish()
{
	...
	out.write((const
				char*)&header,sizeof(PosixTarHeader));
	out.write((const
				char*)&header,sizeof(PosixTarHeader));
	out.flush();
}
